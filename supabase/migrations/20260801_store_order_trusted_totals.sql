...
begin
  ...
  loop
    ...
    where id = v_product_id and product_kind = 'standard';
  ...
  values (
    ...
    coalesce(nullif(btrim(v_profile.email), ''), ''), coalesce(nullif(btrim(v_profile.phone), ''), nullif(btrim(p_shipping_address->>'phone'), ''))
