-- Step 9: let each product variant reference its own gallery image, and make
-- the "in stock" checkbox the single source of truth for availability instead
-- of requiring a stock quantity to be entered.
-- Safe to re-run.

alter table public.product_variants
  add column if not exists image_id uuid references public.product_images(id) on delete set null;

create index if not exists product_variants_image_id_idx
  on public.product_variants (image_id);

-- Availability checks used to require stock_quantity >= quantity even though
-- stock_quantity is optional/informational. From now on the in_stock flag on
-- the product (or variant) alone determines whether an item can be ordered;
-- stock_quantity, when present, is only used to cap the quantity and is never
