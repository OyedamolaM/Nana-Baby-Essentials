...
begin
  ...
  if v_profile.id is null or v_profile.deleted_at is not null or coalesce(v_profile.account_status, 'active') <> 'active' or coalesce(btrim(v_profile.phone), '') = '' then raise exception 'Your account is not available for checkout.'; end if;
  ...
  values (auth.uid(), round(v_subtotal + v_shipping_fee, 2), 'pending', p_shipping_address, p_billing_address, v_order_items, 'paystack', btrim(p_shipping_tier), coalesce(nullif(btrim(v_profile.full_name), ''), nullif(btrim(p_shipping_address->>'name'), ''), 'Customer'), coalesce(nullif(btrim(v_profile.email), ''), ''), coalesce(nullif(btrim(v_profile.phone), ''), nullif(btrim(p_shipping_address->>'phone'), '')))
